...
`AiExecutionGateway`. It creates one query embedding per binding only when all
...
state. `WhitespaceTokenCounter` and the in-memory snapshot/search ports are
deterministic test fakes; production composition must supply a model-compatible
token counter and durable search/snapshot adapters.
